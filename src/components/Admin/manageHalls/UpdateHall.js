...
class UpdateHall extends React.Component {
    ...
    componentDidMount = async()=> {
        await HallService.getById(this.state.id)
            .then((result) => {
    ...
    correctName = async () => {
        if ((this.state.newHall.name) === "") {
            ...
            const isNameFree = await HallService.isNameFree(this.state.newHall.name);
            if(!isNameFree) {
    ...
    render() {
        return(
            <div className="login_block">
                ...
                <div>
                    {(this.state.updateHallFailed) && <a style={{color: 'red'}}>An unknown error occurred
                ...
                <form onSubmit={this.updateHall}>
                    <div>
                        <h3>Name:</h3>
                        <input defaultValue={this.state.hall.name} onChange={e => this.changeHandler(e) }
                    ...
                    <div>
                        <h3>Rows:</h3>
                        <input defaultValue={this.state.hall.rowsAmount} onChange={e => this.changeHandler(e) }
                    ...
                    <div>
                        <h3>Places:</h3>
                        <input defaultValue={this.state.hall.places} onChange={e => this.changeHandler(e) }
